...
<%
	...
	String updateSql = 
		"update board set category_name=?, board_title=?, board_content=?, update_date = NOW() where board_no=? and board_pw=?";
	// UPDATE SQL 쿼리 저장
	...
	if(row == 0) {
		...
		response.sendRedirect(request.getContextPath() + "./board/deleteBoardForm.jsp?boardNo=" + b.boardNo);
	} else if(row == 1) {
